'''
  Loads a .wrl file generated by exportToBlender.m
  Update the filepath of doLoad

  based on import_x3d from blender
  (Adds backward compatability for Blender older than 2.63 (tessface))
  Note:
    (*) Press F12 to see the render result after running the script
    (*) Play with camera orthographic scale to adjust the zoom lavel
  @author Jonathan Pokrass
'''
import bpy;

def doLoad(filepath = r"c:\blue3.wrl"):
    createNewScene = True;
    specularIntensity = 0.0;
    addHemiLight = True;
    addShadowPlane = True;
    addShadowLight = True;
    useMatlabLights = False;
    showLines = True;
    ...
    matScene = bpy.context.scene;
    
    #import scene
    global_matrix = axis_conversion(from_forward='Y', from_up='Z').to_4x4()
    keywords = {};
    keywords["global_matrix"] = global_matrix
    keywords["filepath"] = filepath
    load(bpy.context, **keywords)
    
    hemiLightExists = False;
    shadowPlaneExists = False;
    shadowLightExists = False;
    #fix imported objects
    for o in matScene.objects:
      if o.type == 'CAMERA':
        matCamera = o;
        matCamera.data.type = 'ORTHO';
        matCamera.data.ortho_scale = 475;
        matCamera.data.clip_end = matCamera.location.length * 2;
        matScene.camera = matCamera;
    
      elif o.type == 'LAMP':
    
        if "Hemi" in o.name:
          hemiLightExists = True;
          continue
        if "ShadowLight" in o.name:
          shadowLightExists = True;
          continue
    
        
        try:
            #o.data.distance = o.location.length * 2
            o.data.falloff_type = 'CONSTANT'
        except:
            pass
        
        if not useMatlabLights:
            o.hide_render = True
                
    
      elif o.type == 'MESH':
        if 'ShadowPlane' in o.name:
          shadowPlaneExists = True;
          continue
        ...
        meshMaterial = None
        try:
          meshMaterial = o.material_slots[0].material;
        except:
          pass
#          bpy.ops.object.material_slot_add()
    
        if meshMaterial == None:
          meshMaterial = bpy.data.materials.new("Mat")
          o.data.materials.append(meshMaterial)
          #o.material_slots[-1].material = meshMaterial
    
        meshMaterial.use_vertex_color_paint = True;
        meshMaterial.use_shadows = False;
        meshMaterial.specular_intensity = specularIntensity;
    
      elif o.type == 'CURVE':
        o.data.bevel_depth = 0.25;
        o.data.extrude = 0.25;
        o.hide_render = not showLines;
        pass;
    
    if addHemiLight and not hemiLightExists:
      bpy.ops.object.lamp_add(type='HEMI', location=(0,6000,0))
      hemiLamp = bpy.context.object
      hemiLamp.name = 'Hemi';
      hemiLamp.scale = [1000, 1000, 1]
    
    if addShadowLight and not shadowLightExists:
      bpy.ops.object.lamp_add(type='SUN', location=(0,6000,0))
      shadowLamp = bpy.context.object
      shadowLamp.name = 'ShadowLamp';
      shadowLamp.data.use_only_shadow = True
      shadowLamp.data.shadow_method = 'RAY_SHADOW'
      shadowLamp.data.shadow_ray_samples = 10
      shadowLamp.data.shadow_soft_size = 300
    
    if addShadowPlane and not shadowPlaneExists:
      bpy.ops.mesh.primitive_plane_add()
      shadowPlane = bpy.context.object
      bpy.ops.object.material_slot_add()
      shadowPlane.name = 'ShadowPlane'
      shadowPlane.scale = [1000, 1000, 1000]    
      
      planeMaterial = bpy.data.materials.new("ShadowMat")
      shadowPlane.material_slots[0].material = planeMaterial;
      
      planeMaterial.use_only_shadow = True;
      planeMaterial.use_transparency = True;
      planeMaterial.alpha = 0.5;
      planeMaterial.transparency_method = 'Z_TRANSPARENCY';
      planeMaterial.use_shadeless = True;
...
def importMesh_IndexedFaceSet(geom, bpyima, ancestry):
    ...
    elif bpyima and ((use_faces and len(bpymesh.faces)) or ((not use_faces) and len(bpymesh.tessfaces))):
                ...
                for i, c in enumerate(fcol):
                    ...
                    if ifs_color_index:
                        try:
                            ...
                            # errfile.write("color_index = %d, vi = %d\n" % (color_index, i));
                ...
                if vcolor_spot:  # use 1 color, when ifs_vcol is []
                    ...
                    if ifs_color_index:
                        if color_index >= len(ifs_color_index):
                            print('\tWarning: per face color index out of range (%d)' % ifs_color_index)
                            color_index = 0
                        else:
                            color_index = ifs_color_index[color_index]
                    try:
    ...
    if geom:
        ...
        if bpydata:
            ...
            if type(bpydata) == bpy.types.Mesh:
                ...
                if (use_faces and bpydata.uv_textures) or (not use_faces and bpydata.tessface_uv_textures):
